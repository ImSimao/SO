...
int main(void) { 
	...
	pid_t sl_pid = fork(); // Criação do processo filho para executar o comando date
	if (sl_pid < 0) {
		fatal_system_error("fork for /bin/sl");
	}
	if (sl_pid == 0) {
		execlp("ls", "-la", (char *)NULL);
		fatal_system_error("exec sl");
	}
	...
	int sl_status = 0; // Variável para armazenar o status do processo filho para o comando sl
	...
	if (waitpid(sl_pid, &sl_status, 0) < 0) { // Espera pelo processo filho para o comando sl
		fatal_system_error("waitpid for /bin/ping");
	}
	...
	print_status("/bin/sl", sl_pid, sl_status); // Imprime o status do processo filho para o comando sl
